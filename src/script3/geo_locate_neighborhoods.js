...
var unknownName = 'unknowns';
var UNKNOWNS = path.join(destPath, "".concat(unknownName, ".csv"));
var unmatchedName = 'unmatched';
var UNMATCHED = path.join(destPath, "".concat(unmatchedName, ".csv"));
var GEOJSON_FILE = 'denver_neighborhoods.geojson';
var records = [];
var unknowns = [];
var unmatched = [];
...
var procesFile = function () { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        ...
        INPUTPATH = path.join('members.csv');
        ...
        parser.on('readable', function () { return __awaiter(void 0, void 0, void 0, function () {
            var record, min, max, randomInRange, zip, address, query, response, neighborhood, point, result_1, row;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        min = 1000;
                        max = 5000;
                        randomInRange = Math.floor(Math.random() * (max - min + 1)) + min;
                        return [4 /*yield*/, delay(randomInRange)];
                    case 1:
                        _a.sent();
                        zip = record['zip'].split('-')[0];
                        address = record['address'].split(',')[0];
                        query = address + ' ' + record['city'] + ' ' + record['state'] + ' ' + zip;
                        return [4 /*yield*/, geoCoder.geocode(query)];
                    case 2:
                        response = _a.sent();
                        ...
                        if (response.length > 0 && response !== (null || undefined)) {
                            ...
                            if (result_1.features[0] && result_1.features[0].properties) {
                                neighborhood = result_1.features[0].properties.NBHD_NAME;
                            }
                            else {
